...
def generate(poseFile, shapeText, textureText):
    ...
    result=sample_model.sample_and_refine(256)
    result=result.permute(0, 2, 3, 1)
    result=result.detach().cpu().numpy()
    result*=255
    result=np.asarray(result[0, :, :, :], dtype=np.uint8)
    cv2.imwrite("Successful.png", result[:, :, ::-1])
...
if __name__ == '__main__':
    ...
    generate(posef, shapeText, textureText)
    print("Complete!")
